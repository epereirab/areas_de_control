...
####  - - - -   - - RESOLVIENDO LA OPTIMIZACION  MAESTRO- - - - - - #######
    ...
    results_master = opt.solve(master_instance)
    ...
    for g in master_instance.GENERADORES:
        for s in master_instance.ESCENARIOS:
            ...
            slave_instance.gen_d_pg[g, s] = round(master_instance.GEN_PG[g, s].value, 2)
            slave_instance.gen_d_resup[g, s] = round(master_instance.GEN_PG[g, s].value, 2)
    ...
    # Printing updated parameter
    slave_instance.gen_d_pg.pprint()

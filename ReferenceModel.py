...
def bounds_gen_pg_scenario(model, g, sg):
    if g == sg:
        ...
        return 0, model.gen_pmax[g]
model.GEN_PG_S = Var(model.GENERADORES, model.SCENARIOS_FALLA_GX,
...
def balance_demanda_generacion_scen_rule(model, b, sg):
    lside = (sum(model.GEN_PG_S[g, sg]
                 for g in model.GENERADORES if model.gen_barra[g] == b and g != sg) +
             sum(model.LIN_FLUJO_S[l, sg]
                 for l in model.LINEAS if model.linea_barB[l] == b and model.linea_available[l]))
    rside = (model.demanda[b] - model.ENS_S[b, sg] +
             sum(model.LIN_FLUJO_S[l, sg]
                 for l in model.LINEAS if model.linea_barA[l] == b and model.linea_available[l]))

    return lside == rside

model.CT_balance_demanda_generacion_scenario = Constraint(model.BARRAS, model.SCENARIOS_FALLA_GX,
                                                          rule=balance_demanda_generacion_scen_rule)
...
model.CT_potencia_minima_scenario = Constraint(model.GENERADORES, model.SCENARIOS_FALLA_GX,
                                               rule=p_min_generadores_scen_rule)
model.CT_potencia_maxima_scenario = Constraint(model.GENERADORES, model.SCENARIOS_FALLA_GX,
                                               rule=p_max_generadores_scen_rule)
